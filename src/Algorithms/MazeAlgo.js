export function genMaze(grid) {
    ...
    for(let rows = 1; rows < 21; rows++) nodes.push(OG[rows][0]);
    for(let rows = 1; rows < 20; rows++) nodes.push(OG[rows][54]);
    for(let cols = 1; cols < 55; cols++) nodes.push(OG[20][cols]);

    for (let cols = 1; cols < 54; cols++) {
        for(let rows = 1; rows < getRandomArbitrary(4, 20); rows++){
            let row = getRandomInt(20)
